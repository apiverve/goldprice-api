...
class GoldpriceValidationException implements Exception {
  final List<String> errors;

  GoldpriceValidationException(this.errors);

  @override
  String toString() => 'GoldpriceValidationException: ${errors.join("; ")}';
}
...
///
/// For more information, visit: https://goldprice.apiverve.com?utm_source=dart&utm_medium=readme
///
/// Parameters:
...
class GoldpriceClient {
  ...
  GoldpriceClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/goldprice',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [GoldpriceValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw GoldpriceValidationException(errors);
    }
  }

  /// Execute a request to the Gold Price API.
  ///
  /// Parameters are validated before sending the request.
  Future<GoldpriceResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw GoldpriceException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return GoldpriceResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw GoldpriceException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw GoldpriceException('Resource not found');
      } else {
        throw GoldpriceException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is GoldpriceException) rethrow;
      throw GoldpriceException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Gold Price API client.
class GoldpriceException implements Exception {
  final String message;

  GoldpriceException(this.message);

  @override
  String toString() => 'GoldpriceException: $message';
}
